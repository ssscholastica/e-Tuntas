  ...
  class _CekAduanBPJSState extends State<CekAduanBPJS> {
    ...
    try {
      ...
      final bool isAuthorized = await _isUserAuthorized(searchQuery);
  ...
  Future<bool> _isUserAuthorized(String searchQuery) async {
    try {
      final currentUser = await _getCurrentUserData();
  ...
  Future<Map<String, dynamic>?> _getCurrentUserData() async {
    try {
      ...
      final String? userNik = prefs.getString('user_nik') ??
          prefs.getString('nik') ??
          prefs.getString('userNik');
      ...
      print(
          "Retrieved from SharedPreferences - userEmail: $userEmail, userNik: $userNik, userData: $userData");
      ...
      if (userData != null && userData.isNotEmpty) {
        try {
          ...
          // Ensure NIK is included in the returned data
          if (userNik != null &&
              userNik.isNotEmpty &&
              !parsedUserData.containsKey('nik')) {
            parsedUserData['nik'] = userNik;
          }
      ...
      // Return basic user data with NIK if available
      return {
        'email': userEmail,
        'is_admin': false,
        'name': 'User',
        if (userNik != null && userNik.isNotEmpty) 'nik': userNik
      };
    } catch (e) {
